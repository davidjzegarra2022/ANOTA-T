    ...
    <div className="space-y-5">
      <div>
        <h2 className="flex items-center gap-2 text-base font-bold text-navy">
          <IconBox className="h-5 w-5 text-brand-dark" />
      ...
      <div className="card p-4">
        <p className="text-sm font-semibold text-navy">Base de datos compartida (Supabase)</p>
            ...
            <div className="flex items-center gap-2">
              ...
              {sbConfig.source === 'local' && (
                <button
                  ...
                  className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-ink hover:bg-surface"
                >
                  Quitar
                </button>
              )}
              {sbSavedMsg && <span className="text-xs font-semibold text-emerald-600">Guardado.</span>}
            </div>
          </div>
        )}
        ...
        {sbConfig.source && (
          <div className="mt-3 flex items-center gap-2 border-t border-slate-200 pt-3">
            ...
            {sbMsg && (
              <span className={`text-xs font-semibold ${sbMsg.ok ? 'text-emerald-600' : 'text-red-600'}`}>
                {sbMsg.text}
              </span>
            )}
          </div>
      ...
      <div className="card border-amber-300 bg-amber-50 p-4">
        <p className="text-sm font-semibold text-navy">Plantilla Excel (recomendado)</p>
        <p className="mt-1 text-xs leading-relaxed text-muted">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <label className="btn btn-primary cursor-pointer text-xs !px-3 !py-2">
        ...
        {excelMsg && (
          <p className={`mt-1 text-xs font-semibold ${excelMsg.ok ? 'text-emerald-600' : 'text-red-600'}`}>
            {excelMsg.text}
          </p>
        )}
          ...
          <div className="mt-3 space-y-2">
            <div className="overflow-x-auto rounded-lg border border-slate-200">
              <table className="w-full min-w-[560px] text-left text-[12px]">
                <thead className="bg-surface text-[10px] tracking-wide text-muted uppercase">
                  <tr>
            ...
            {excelRows.length > excelPreview.length && (
              <p className="text-[11px] text-muted">… y {excelRows.length - excelPreview.length} fila(s) más.</p>
            )}
            <button
              type="button"
            ...
            {!sbConfig.source && (
              <p className="text-[11px] text-brand-dark">Configura Supabase arriba antes de guardar.</p>
            )}
          </div>
        )}
      </div>
      ...
      <form onSubmit={handleAddOne} className="card p-4">
        <p className="text-sm font-semibold text-navy">Agregar una agencia</p>
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <label className="block">
            <span className="mb-1 block text-xs font-semibold text-ink">
              Courier<span className="text-red-600"> *</span>
            </span>
            <select
            ...
            >
              ...
              <option value={NEW_COURIER}>+ Nueva empresa…</option>
            </select>
          </label>
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          {addMsg && (
            <span className={`text-xs font-semibold ${addMsg.ok ? 'text-emerald-600' : 'text-red-600'}`}>
              {addMsg.text}
            </span>
          )}
        </div>
      ...
      <details className="card p-4">
        ...
        <div className="mt-3 flex flex-wrap items-end gap-3">
          <label className="block">
            <span className="mb-1 block text-xs font-semibold text-ink">Formato</span>
            <select
              value={importFormat}
              onChange={(e) => setImportFormat(e.target.value)}
              className="input-field"
            >
              <option value="listado">Listado (texto pegado)</option>
              <option value="csv">CSV</option>
              <option value="json">JSON</option>
            </select>
          </label>
          <label className="block">
            <span className="mb-1 block text-xs font-semibold text-ink">Courier por defecto</span>
            <select
              value={importCourier}
          ...
          <label className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-ink transition hover:bg-surface">
        ...
        <p className="mt-2 text-[11px] leading-relaxed text-muted">{FORMAT_HELP[importFormat]}</p>
        <textarea
          value={importText}
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          {importMsg && (
            <span className={`text-xs font-semibold ${importMsg.ok ? 'text-emerald-600' : 'text-red-600'}`}>
              {importMsg.text}
            </span>
          )}
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 pt-3">
          <p className="text-sm font-semibold text-navy">Agencias propias en este dispositivo ({custom.length})</p>
          <div className="flex items-center gap-2">
            <button
              type="button"
      ...
      {sbConfig.source && sbRows.length > 0 && (
        <div className="card p-4">
          <p className="text-sm font-semibold text-navy">Borrar por courier en Supabase</p>
          <p className="mt-1 text-xs text-muted">Afecta a todos los visitantes — úsalo solo para corregir una carga equivocada.</p>
